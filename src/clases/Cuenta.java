...
public class Cuenta {
    ...
    public Cuenta(long numero_cuenta, long numero_identificacion, double saldo_actual) throws NoNegativoExcepcion, NoCeroExcepcion {
        if(numero_cuenta < 0){
            throw new NoNegativoExcepcion("EL Numero de  cuenta no puede ser negativo");
        }
        if(numero_cuenta == 0){
            throw new NoCeroExcepcion("El numero de cuenta no debe ser cero");
        }
        if(saldo_actual < 0){
            throw new NoNegativoExcepcion("El numero de cuneta no puede ser negativo");
        }
        if(numero_identificacion < 0){
            throw new NoNegativoExcepcion("El numero de identificacion no debe ser negativo");
            
        }
        if(numero_identificacion == 0){
            throw new NoNegativoExcepcion("El numero de identificacion no debe ser cero");
